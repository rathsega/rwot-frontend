...
function Disbursement({ cases, handleRefresh, searchTerm = "", onSearchChange }) {
  const navigate = useNavigate();
  const { token } = useAuth() || {};

  // Export to Excel
  const handleExport = () => {
    exportLeadsToExcel(cases, "disbursement_leads.xlsx");
  };

  // Define actions for CaseCard - Disbursement only has view
      ...
      <div className="case-cards-grid">
        {cases.length === 0 ?
          ...
          >
            ...
            No records found in Disbursement.
          </div>
