...
import { FaEdit, FaArrowRight, FaFileSignature } from "react-icons/fa";
import ServerSearchBar from "../../common/ServerSearchBar";

function Underwriting({ cases, handleRefresh, searchTerm = "", onSearchChange }) {
  const navigate = useNavigate();
  const { token } = useAuth() || {};

  // Export to Excel
  const handleExport = () => {
    exportLeadsToExcel(cases, "underwriting_leads.xlsx");
  };
  ...
  const handleMoveToNextStage = async (caseData) => {
    try {
      await apiFetch(`/cases/${caseData.caseid}/status`, {
        method: "PATCH",
        token,
        body: JSON.stringify({ status: "One Pager" })
      });
      handleRefresh?.();
    } catch (err) {
      ...
      <div className="case-cards-grid">
        {cases.length === 0 ?
          ...
          >
            ...
            No records found in Underwriting.
          </div>
