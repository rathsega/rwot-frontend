...
function Accept({ cases, handleRefresh, searchTerm = "", onSearchChange }) {
  const navigate = useNavigate();
  const { token } = useAuth() || {};

  // Export to Excel
  const handleExport = () => {
    exportLeadsToExcel(cases, "accepted_leads.xlsx");
  };

  // Schedule meeting action
  const handleScheduleMeeting = async (caseData) => {
    try {
      await apiFetch(`/cases/${caseData.caseid}/status`, {
        method: "PATCH",
        token,
        body: JSON.stringify({ status: "Meeting Scheduled" })
      });
      handleRefresh?.();
    } catch (err) {
