...
function DocumentationInitiated({ cases, handleRefresh }) {
  ...
  // Export to Excel
  const handleExport = () => {
    exportLeadsToExcel(cases, "documentation_initiated_leads.xlsx");
  };

  // Progress action - move to Documentation In Progress
  const handleProgress = async (caseData) => {
    try {
      await apiFetch(`/cases/${caseData.caseid}/status`, {
        method: "PATCH",
        token,
        body: JSON.stringify({ status: "Documentation In Progress" })
      });
      handleRefresh?.();
    } catch (err) {
      console.error("Failed to progress documentation:", err);
    }
  };
      ...
      <div className="case-cards-grid">
        {filteredLeads.length === 0 ?
          ...
          >
            ...
            No records found in Documentation Initiated.
          </div>
...
export default DocumentationInitiated;