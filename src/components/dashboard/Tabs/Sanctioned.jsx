...
import { FaEdit, FaMoneyBillWave, FaCheckCircle } from "react-icons/fa";
import ServerSearchBar from "../../common/ServerSearchBar";

function Sanctioned({ cases, handleRefresh, searchTerm = "", onSearchChange }) {
  const navigate = useNavigate();
  const { token } = useAuth() || {};

  // Export to Excel
  const handleExport = () => {
    exportLeadsToExcel(cases, "sanctioned_leads.xlsx");
  };

  // Disburse action
  const handleDisburse = async (caseData) => {
    try {
      await apiFetch(`/cases/${caseData.caseid}/status`, {
        method: "PATCH",
        token,
        body: JSON.stringify({ status: "Disbursement" })
      });
      handleRefresh?.();
    } catch (err) {
      console.error("Failed to move case to disbursement:", err);
    }
  };
      ...
      <div className="case-cards-grid">
        {cases.length === 0 ?
          ...
          >
            ...
            No records found in Sanctioned.
          </div>
